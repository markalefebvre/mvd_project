...
def get_frame(frame,dummies_df):
    ...
    frame = frame.groupby(['Unit','Week'])['Wait_Time'].mean().reset_index()
    z = pd.merge(frame,dummies_df, on='Unit', how='inner')
    z = z.groupby(["Week"]).apply(lambda x: x.sort_values(["Unit"], ascending = False)).reset_index(drop=True)
    
    return z
    

def test_set(frame):
    z = frame[frame['Week']==frame['Week'].max()]
